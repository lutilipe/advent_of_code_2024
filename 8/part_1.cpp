...
void insertAntinode(vector<vector<char>>& a, int i, int j, int& count) {
    if (!isInBounds(a, i, j)) return;
    if (a[i][j] != '#') {
        a[i][j] = '#';
        count++;
    }
        ...
        REPL(i, 0, antenna.S.size()) {
            ...
            REPL(j, 0, antenna.S.size()) {
                ...
                if (curr.F > toCompare.F && curr.S > toCompare.S) {
                    ...
                    insertAntinode(antinodes, curr.F, curr.S + dy, count);
                    insertAntinode(antinodes, toCompare.F, toCompare.S - dy, count);
                } else if (curr.F < toCompare.F && curr.S > toCompare.S) {
                    insertAntinode(antinodes, curr.F - dx, curr.S + dy, count);
                    insertAntinode(antinodes, toCompare.F + dx, toCompare.S - dy, count);
                } else if (curr.F < toCompare.F && curr.S == toCompare.S) {
                    insertAntinode(antinodes, curr.F - dx, curr.S, count);
                    insertAntinode(antinodes, toCompare.F + dx, toCompare.S, count);
                } else if (curr.F < toCompare.F && curr.S < toCompare.S) {
                    insertAntinode(antinodes, curr.F - dx, curr.S - dy, count);
                    insertAntinode(antinodes, toCompare.F + dx, toCompare.S + dy, count);
                } else if (curr.F == toCompare.F && curr.S < toCompare.S) {
